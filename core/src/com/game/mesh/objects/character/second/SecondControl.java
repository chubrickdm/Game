...
public class SecondControl extends SecondBody{
	...
	private void updateControl (){
		...
		if (Gdx.input.isKeyJustPressed (Input.Keys.TAB)){
			ObjectManager.getInstance ().addMessage (new CharacterChangeMessage (second));
		}
		else if (deltaX != 0 || deltaY != 0){
			isMove = true;
			ObjectManager.getInstance ().addMessage (new MoveMessage (second, deltaX, deltaY, bodySecond.getBodyX (),
					bodySecond.getBodyY (), bodySecond.getSpriteX (), bodySecond.getSpriteY (), bodySecond.getBodyW (),
					bodySecond.getBodyH ()));
			bodySecond.move (deltaX, deltaY);
		}
	}
	
	
	public SecondControl (SecondBody second){
		this.second = second;
	}
